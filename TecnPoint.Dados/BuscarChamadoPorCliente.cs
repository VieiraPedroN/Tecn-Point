...
namespace TecnPoint.Dados.BuscarChamadoPorCliente
{
    public class BuscarChamadoPorCliente
    {
        public List<ExibicaoChamado> ExibeChamadosCliente(int IdUsuario, string TipoUsuario)
        {
            ...
            {
                string queryBuscaChamadosCliente = "SELECT c.Titulo, cliente.Nome AS NomeCliente, funcionario.Nome AS NomeFuncionario, c.Status " +
                                              "FROM Chamados c " +
                                              "JOIN Usuarios ON c.fk_idCliente = cliente.id_Usuario " +
                                              "JOIN Usuarios ON c.fk_idFuncionario = funcionario.id_Usuario " +
                                              "WHERE ";
                if(TipoUsuario == "Cliente")
                {
                    queryBuscaChamadosCliente += "id_fkCliente = @RecebeIdUsuario";
                }
                else
                {
                    queryBuscaChamadosCliente += "id_fkFuncionario = @RecebeIdUsuario";
                }
