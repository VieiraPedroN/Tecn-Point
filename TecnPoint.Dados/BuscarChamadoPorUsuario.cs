...
namespace TecnPoint.Dados.BuscarChamadoPorUsuario
{
    public class BuscarChamadoPorUsuario
    {
        public List<ExibicaoChamado> ExibeChamadosUsuario(int IdUsuario, string TipoUsuario)
        {
            ...
            {
                string queryBuscaChamados = "SELECT chamados.Id_Chamado, " +
                                                     "chamados.Titulo, " +
                ...
                {
                    queryBuscaChamados += "WHERE fk_idCliente = @RecebeIdUsuario";
                }

                using (NpgsqlCommand comandoConsulta = new NpgsqlCommand(queryBuscaChamados, conexaoBuscaChamado.conexao))
                {
                    if (TipoUsuario == "Cliente")
